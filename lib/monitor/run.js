...
var rvn_cfg=process.env.RVN
console.log("Using Raven config", rvn_cfg)
Raven.config(rvn_cfg).install()
  ...
  child.on('exit', function (code, signal) {
    ...
    if (signal === config.signal || code === 0) {
      ...
      if (options.exitcrash) {
        ...
        ' starting...');
        utils.log.fail('hacked tramitapp, restarting anyway')
        utils.log.fail("Using Raven config", rvn_cfg)

        Raven.captureException(new Error("server nodemon error"), (err, event_id){
          if(err) utils.log.fail("Raven did not work", err)
          else utils.log.fail("Raven worked, event id", event_id)
          setTimeout(function(){
            restart()
          }, 1000)
